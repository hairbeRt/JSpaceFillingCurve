...
public class DaunCompare implements Comparator<Point2D> {
	...
	//Window is always a rectangle oriented at the coordinate axes from the origin with a specifiec height and width
	int width;
	int height;
	...
	public DaunCompare(Point2D origin, int width, int height) {
		...
		tilingTriangles[9] = new Point2D[] {origin.add(a+b,a+b), origin.add(a+b,a), origin.add((2*a)+b, a+b)};
		tilingTriangles[10] = new Point2D[] {origin.add((2*a)+b,a+b), origin.add((2*a)+b, 4*a), origin.add(a+b,a+b)};
		
		tilingTriangles[11] = new Point2D[] {origin.add(2*(a+b), 4*a), origin.add((2*a)+b, 4*a), origin.add(2*(a+b), 3*a)};
		tilingTriangles[12] = new Point2D[] {origin.add(2*(a+b), 3*a), origin.add((2*a)+b, 3*a), origin.add(2*(a+b), 2*a)};
		tilingTriangles[13] = new Point2D[] {origin.add(2*(a+b), 2*a), origin.add((2*a)+b, 2*a), origin.add(2*(a+b), a)};
		
		tilingTriangles[14] = new Point2D[] {origin.add(4*b, a), origin.add(4*b, a+b), origin.add(2*(a+b), a)};
		tilingTriangles[15] = new Point2D[] {origin.add(2*(a+b), 4*a), origin.add(2*(a+b), a+b), origin.add(4*b, 4*a)};
		
		/*For nonstandard traversals of the Daun tiling, you can permute
		 * both the entries of tilingTriangles for the order of tilings
		 * as well as each tiling Triangle to modify the orientation
		 * 
		 */
	...
	public int determineTile(Point2D A) {
		...
		if(relativeY <= a) return relativeX/b;
		if(relativeX <= a) return 4 + (relativeY-a)/b;
		if(relativeX <= a+b) return 6 + (height-relativeY)/a;
		if(relativeX <= 2*a + b) return 9 + (relativeY-a)/b;
		if(relativeX <= 2*(a+b)) return 11 + (height-relativeY)/a;
	...
	public int compare(Point2D A, Point2D B) {
		...
		while(true) {
			if(aPoint.equals(bPoint)) System.out.print("Big oops");
			
			aTile = determineTile(aPoint);
			bTile = determineTile(bPoint);
			
			if(aTile != bTile) return bTile - aTile;
