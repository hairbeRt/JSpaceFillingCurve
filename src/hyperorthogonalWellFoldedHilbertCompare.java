...
public class hyperorthogonalWellFoldedHilbertCompare implements Comparator<PointnD>{
	...
	public hyperorthogonalWellFoldedHilbertCompare(PointnD lowerLeft, PointnD upperRight, int dim) {
		if(lowerLeft.getDimension() != dim || upperRight.getDimension() != dim) throw new java.lang.ArrayIndexOutOfBoundsException();
	...
	public int compare(PointnD A, PointnD B) {
		if(A.getDimension() != this.dimension || B.getDimension() != this.dimension) throw new ArrayIndexOutOfBoundsException();
		
		
		/*
		 * Maybe for funny use I thought you could make p[0] and q[0] the absolute values
		 * could be useful
		 */
		...
		q[0] = 0;
		for(int i = 1; i <= dimension; i++) {
			p[i] = (A.getCoord(i-1)-lowerLeft.getCoord(i-1))/(double)(upperRight.getCoord(i-1)-lowerLeft.getCoord(i-1));
			q[i] = (B.getCoord(i-1)-lowerLeft.getCoord(i-1))/(double)(upperRight.getCoord(i-1)-lowerLeft.getCoord(i-1));
			unsgnedPrm[i] = i;
			sgnsInvPrm[i] = 1;
		}
		 ...
		 */
		while(!Arrays.equals(p,q) && distance(p,q) > 0.005) {
