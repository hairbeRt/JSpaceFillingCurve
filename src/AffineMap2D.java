...
public class AffineMap2D {
	...
	public Point2D apply(Point2D P) {
		return new Point2D(
				(int) Math.round((coefficients[0] * P.getX()) + (coefficients[2] * P.getY()) + coefficients[4]),
	...
	public static AffineMap2D getMapFromTriangle(Point2D[] srcPoints, Point2D[] dstPoints) {
		...
		AffineTransform UnitCoordsToSource = new AffineTransform(srcVec1.getX(), srcVec1.getY(), srcVec2.getX(),
				srcVec2.getY(), srcBase.getX(), srcBase.getY());

		AffineTransform UnitCoordsToDestination = new AffineTransform(dstVec1.getX(), dstVec1.getY(), dstVec2.getX(),
				dstVec2.getY(), dstBase.getX(), dstBase.getY());

		// It is the responsibility of the programmer to guarantee that we are
		// indeed dealing with actual triangles (such that its defining matrix
		// is invertible) lol
		try {
